...
function updatePlayers(teamId, teamType) {
    ...
    xhr.open("GET", "getPlayersByTeamJson?team_id=" + teamId, true);
    console.log(xhr);
            ...
            if (xhr.status == 200) {
                try {
                    ...
                    players.forEach(function(player) {
                        ...
                        playersHtml += "<input type='number' id='player_" + player.id + "_goals' name='player_" + player.id + "_goals' required min='0'>";
                        playersHtml += "</div>";
                        playersHtml += "<div class='stat-item'>";
                        playersHtml += "<label for='player_" + player.id + "_assists'>Asisturi:</label>";
                        playersHtml += "<input type='number' id='player_" + player.id + "_assists' name='player_" + player.id + "_assists' required min='0'>";
                        playersHtml += "</div>";
                        playersHtml += "</div>";
                        playersHtml += "</div>";
                    });
